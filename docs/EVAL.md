...
final audit check -> `python scripts/final_audit.py` validates required structure, README content, marker scan, secret history, backend tests, compileall, drift eval, and frontend module syntax. Latest live run with `FINAL_AUDIT_LIVE=1` returned `final_audit=ok`, including demo team `demo-7553c801`, resolved conflict `decision-aa9f8e463d`, 2 recall citations, 2 brief citations, Qdrant collection status `green`, and orchestration `adk parallel extraction, sequential drift write`.
fresh clone audit check -> `python scripts/fresh_clone_audit.py` clones the current repo into a temporary directory, rewrites compose ports to avoid collisions, points the cloned frontend config at the cloned backend, starts the cloned Docker stack, runs `scripts/demo_walkthrough.py`, and tears the stack down. Latest pre-commit worktree run returned `{"fresh_clone_audit":"ok","project":"meetingmatefresh870a3487","api_port":18135,"frontend_port":15183,"qdrant_port":16385,"team_id":"demo-14ca0afd","conflict_id":"decision-0ec4d4078a","orchestration":"adk parallel extraction, sequential drift write"}`.
```
