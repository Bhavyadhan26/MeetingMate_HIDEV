...
MiniLM embedding check -> rebuilt backend image imports `sentence_transformers`, `embed_text('Qdrant vector memory')` returns 384 dimensions with normalized vector norm `1.0`, and `QdrantVectorMemory` recreates old 64-dimension collections when `QDRANT_RECREATE_ON_VECTOR_SIZE_MISMATCH` is enabled
