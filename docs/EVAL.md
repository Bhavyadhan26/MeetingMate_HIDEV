...
Lyzr Studio RAG config check -> `python scripts/lyzr_rag_check.py` returned `lyzr_rag_check: ok` for RAG id `6a5f4cfc7f75d333678d6297`, collection `decisions_vector_kboi3b`, provider `Qdrant [MeetingMate_Qdrant]`, LLM `gpt-4o-mini`, embedding `text-embedding-ada-002`, semantic data model `false`, and multi-tenancy `true`; Lyzr Studio traces are produced by invoking a Lyzr agent attached to that Knowledge Base
